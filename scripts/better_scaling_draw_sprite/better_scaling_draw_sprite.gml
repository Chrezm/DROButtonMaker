...
function better_scaling_draw_sprite() {
	...
	var _scale_up = ((argument[4] > 1 || argument[5] > 1) && argument[9] != 0), _scale_down = (argument[4] < 1 || argument[5] < 1);

	if (_scale_up) {
	    var _texture = sprite_get_texture(argument[0], argument[1]);
	    switch (argument[9]) {
	        case 1:
	            shader_set(sh_better_scaling_bicubic);
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_bicubic, "texel_size"), texture_get_texel_width(_texture), texture_get_texel_height(_texture));
	            texture_set_interpolation(true);
	            break;
            
	        case 2:
	            shader_set(sh_better_scaling_hq4x);
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_hq4x, "texel_size"), texture_get_texel_width(_texture), texture_get_texel_height(_texture));
	            texture_set_interpolation(false);
	            break;
            
	        case 3:
	            shader_set(sh_better_scaling_5xbra);
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbra, "texel_size"), texture_get_texel_width(_texture), texture_get_texel_height(_texture));
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbra, "texture_size"), 1 / texture_get_texel_width(_texture), 1 / texture_get_texel_height(_texture));
	        ...
	        case 4:
	            ...
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbrb, "texel_size"), texture_get_texel_width(_texture), texture_get_texel_height(_texture));
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbrb, "texture_size"), 1 / texture_get_texel_width(_texture), 1 / texture_get_texel_height(_texture));
	        ...
	        case 5:
	            ...
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbrc, "texel_size"), texture_get_texel_width(_texture), texture_get_texel_height(_texture));
	            shader_set_uniform_f(shader_get_uniform(sh_better_scaling_5xbrc, "texture_size"), 1 / texture_get_texel_width(_texture), 1 / texture_get_texel_height(_texture));
	    ...
	    var _texture, _width, _height, _samples_x, _samples_y, _max_samples, _offset_x, _offset_y;
	    _texture = sprite_get_texture(argument[0], argument[1]);
	    _width = sprite_get_width(argument[0]); _height = sprite_get_height(argument[0]);
	    if (argument[4] == 0) _samples_x = _width; else _samples_x = min(_width, 1 / argument[4]);
	    if (argument[5] == 0) _samples_y = _height; else _samples_y = min(_height, 1 / argument[5]);
	    _max_samples = max(_samples_x, _samples_y);
	    _offset_x = texture_get_texel_width(_texture) * _samples_x * 0.5; _offset_y = texture_get_texel_height(_texture) * _samples_y * 0.5;
	    if (_max_samples <= 2) {
