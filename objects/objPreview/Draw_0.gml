...
if (character_name != "") {
	if !surface_exists(preview_surface) {
		preview_surface = surface_create_based_on_camera(0);
	}
	...
	if (sprite_exists(mask_sprite)) {
		...
		var _negative_mask_sprite = sprite_create_from_surface(mask_surface, 0, 0, preview_size, preview_size, 0, 0, 0, 0);
		surface_set_target(preview_surface);
		gpu_set_blendmode(bm_subtract);
		shader_set(sh_invert_alpha);
		draw_sprite(_negative_mask_sprite, 0, _x, _y);
		shader_reset();
		gpu_set_blendmode(bm_normal);
		surface_reset_target();
		sprite_delete(_negative_mask_sprite);
	}
